...
class Main:
    ...
    async def process_city(self, city_data: dict):
        ...
        if city is not None:
            self.log(f"City already exists: {
        ...
        async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(limit_per_host=100, limit=1000)) as session:
            try:
                ...
                self.log(f"Error processing city {
    ...
    async def run(self):
        self.log("Initializing database...", level="info")

        await self.db.initialize()

        self.log("Database initialized.", level="info")

        self.log("Fetching cities from IBGE API...", level="info")
