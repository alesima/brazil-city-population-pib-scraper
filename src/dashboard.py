...
class Visualization:
    def plot_pib_per_capita(self, df):
        return px.bar(
            df, x="nome", y="pib_per_capita", title="PIB Per Capita por Município"
        )
...
class Filter:
    def apply_filters(
        ...
        filtered_df = df[
            ...
            & (df["pib_per_capita"] >= pib_min)
            & (df["pib_per_capita"] <= pib_max)
        ...
        if query:
            filtered_df = filtered_df[
                filtered_df["nome"].str.contains(query, case=False)
            ]
...
class Sidebar:
    ...
    def apply_filters(self, df):
        ...
        estado = st.sidebar.multiselect(
            "Selecione o(s) Estado(s)", df["estado"].unique()
        )
        pop_min, pop_max = st.sidebar.slider(
            "População",
            int(df["populacao"].min()),
            int(df["populacao"].max()),
            (int(df["populacao"].min()), int(df["populacao"].max())),
        )
        pib_min, pib_max = st.sidebar.slider(
            "PIB Per Capita",
            float(df["pib_per_capita"].min()),
            float(df["pib_per_capita"].max()),
            (float(df["pib_per_capita"].min()), float(df["pib_per_capita"].max())),
        )
        ...
        return self.filter_obj.apply_filters(
            df, query, estado, pop_min, pop_max, pib_min, pib_max, cicc, gcm, samu
        )
...
class Comparison:
    ...
    def display(self, filtered_df):
        ...
        if selected_cities:
            comparison_df = filtered_df[filtered_df["nome"].isin(selected_cities)]
