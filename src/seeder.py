...
class Seeder:
    ...
    async def process_city(self, city_data: dict):
        ...
        if city is not None:
            log(f"City already exists: {city_name}/{state_code} (ID: {city_id})")
            return

        log(f"Processing city: {city_name}/{state_code} (ID: {city_id})")

        async with aiohttp.ClientSession(
            connector=aiohttp.TCPConnector(limit_per_host=100, limit=1000)
        ) as session:
            try:
                population, pib_per_capita = await self.city_scraper.fetch_city_data(
                    session, city_name, state_code
                )
            except Exception as e:
                log(
                    f"Error processing city {city_name}/{state_code}: {str(e)}",
                    level="error",
                )
                return

        if population is None or pib_per_capita is None:
            log(f"Failed to fetch data for {city_name}/{state_code}", level="warning")
            return
        ...
        log(
            f"Saved: {city_name}/{state_code} - Pop: {population}, PIB: {pib_per_capita}",
            level="info",
        )
