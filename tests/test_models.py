...
def test_municipio_table_defaults(session):
    municipio = Municipio(nome="TestCity", estado="TS",
                          populacao=10000, pib_per_capita=50000.0)
    session.add(municipio)
    session.commit()

    fetched_municipio = session.query(
        Municipio).filter_by(nome="TestCity").first()
...
def test_municipio_unique_id(session):
    municipio1 = Municipio(nome="CityOne", estado="A",
                           populacao=1000, pib_per_capita=20000.0)
    municipio2 = Municipio(nome="CityTwo", estado="B",
                           populacao=2000, pib_per_capita=30000.0)
