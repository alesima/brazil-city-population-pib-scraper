...
async def test_fetch_city_data_success(mock_soup, mock_get, scraper):
    ...
    mock_soup.return_value.find_all.return_value = [
        BeautifulSoup('<div class="indicador"><p>População residente</p><p class="ind-value">1.234.567</p></div>',
                      'html.parser').find('div', class_='indicador'),
        BeautifulSoup('<div class="indicador"><p>PIB per capita</p><p class="ind-value">R$ 12.345,67</p></div>',
                      'html.parser').find('div', class_='indicador')
    ]

    async with aiohttp.ClientSession() as session:
        population, pib_per_capita = await scraper.fetch_city_data(session, "TestCity", "TS")
